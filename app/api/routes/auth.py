from fastapi import APIRouter, Depends, HTTPException, status, Depends, Request, Response
import httpx
from models.model import TokenExchangeRequest, User
from db.session import get_session
import os
from dotenv import load_dotenv
from services.security import create_access_token, create_refresh_token, get_current_user, get_user_browser
...
@router.post("/web/exchange")
@limiter.limit("5/minute")
async def web_login_exchange(request: Request, response: Response, payload: dict, session: Session = Depends(get_session)):
    token_url = "https://github.com/login/oauth/access_token"
    ...
    async with httpx.AsyncClient() as client:
        github_response = await client.post(token_url, json=payload, headers=headers)
